...
const pdfLogicStart = content.indexOf('if (filename.endsWith(\'.pdf\') || filename.endsWith(\'.txt\')) {', startIdx);
const endPdfLogic = content.indexOf('      addLog(`Generated and opened full PDF deliverable report with bundled calculation details & plots`, \'success\');', pdfLogicStart);

const newLogic = `if (filename.endsWith('.pdf') || filename.endsWith('.txt')) {
...
const newContent = content.substring(0, pdfLogicStart) + newLogic + content.substring(endPdfLogic);
